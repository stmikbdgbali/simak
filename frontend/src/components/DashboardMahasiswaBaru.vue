				...
				<v-timeline align-top :dense="$vuetify.breakpoint.smAndDown">
					...
					>
						<v-card color="red lighten-2" dark>
							...
							<v-card-text class="white text--primary" v-else>
								...
								<p>
									Biaya Pendaftaran: Rp. {{ biaya_pendaftaran | formatUang }}.
								</p>
								<v-alert type="info">
					...
					>
						<v-card color="purple darken-1" dark>
							...
							<v-card-text class="white text--primary">
								Setelah melakukan ujian mohon lengkapi persyaratan berikut ini
								dengan mengunggah ke sistem:
								<ul>
									<li>Scan Pas Foto</li>
					...
					>
						<v-card color="red lighten-2" dark>
							<v-card-title class="title">Konfirmasi Pembayaran</v-card-title>
							<v-card-text class="white text--primary">
								<p>Konfirmasi Pembayaran Biaya Daftar Ulang yang meliputi.</p>
								<ol>
									<li>Jas Almamater + PSPT</li>
									<li>Biaya Pengembangan</li>
									<li>SPP Bulan September</li>
								</ol>
								<v-alert type="info">
									Hubungi petugas pendaftaran bila Biaya Pengembangan tidak
			...
			<v-col xs="12" sm="6" md="4">
				<v-card outlined>
					...
					<v-card-subtitle class="justify-center">
						Silahkan hubungi kami, bila ada pertanyaan atau hal yang belum jelas
					...
					<v-card-text class="justify-center">
						...
						<div>
							...
							<div>
								<v-icon>mdi-card-account-phone-outline</v-icon> 0878 6111 0357
							</div>
						</div>
						<div>
							<h3 class="headline pink--text text--accent-2">VIONA</h3>
							<div>
								<v-icon>mdi-card-account-phone-outline</v-icon> 0823 5935 5795
							</div>
						</div>
					</v-card-text>
				</v-card>
	...
	export default {
		...
		methods: {
			initialize: async function() {
				...
				await this.$ajax
					.get("/spmb/ujianonline/peserta/" + user_id, {
						headers: {
				...
				await this.$ajax
					.get("/spmb/formulirpendaftaran/" + user_id, {
						headers: {
							Authorization: this.$store.getters["auth/Token"],
						},
					})
