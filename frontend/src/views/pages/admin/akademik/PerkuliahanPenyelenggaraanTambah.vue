		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Halaman untuk melakukan penyelenggaraan matakuliah per prodi, tahun
					akademik, dan semester.
				</v-alert>
			</template>
		</ModuleHeader>
		<v-container fluid>
							...
							<v-card-actions>
								...
								<v-btn
									...
									:disabled="
										!form_valid ||
											btnLoading ||
											!daftar_matkul_selected.length > 0
									"
								>
									SIMPAN
								</v-btn>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.daftar_ta = this.$store.getters["uiadmin/getDaftarTABefore"](
				this.tahun_akademik
			);
		...
		methods: {
			...
			fetchMatkul: async function() {
				...
				await this.$ajax
					.post(
						"/akademik/matakuliah/penyelenggaraan",
						{
							prodi_id: this.prodi_id,
							ta_matkul: this.tahun_akademik,
			...
			save: async function() {
				if (this.$refs.frmdata.validate()) {
					...
					await this.$ajax
						.post(
							"/akademik/perkuliahan/penyelenggaraanmatakuliah/store",
							{
								prodi_id: this.prodi_id,
								ta: this.tahun_akademik,
								semester_akademik: this.semester_akademik,
								matkul_selected: JSON.stringify(
									Object.assign({}, this.daftar_matkul_selected)
								),
							},
							{
								headers: {
