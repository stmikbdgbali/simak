		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Berisi pendaftar baru, silahkan melakukan filter tahun akademik dan program studi. CATATAN: Melakukan perubahan terhadap Prodi, 
					Kelas, dan Tahun Pendaftaran Mahasiswa Baru tidak berpengaruh terhadap Transaksi keuangan yang telah dilakukannya.
				</v-alert>
			</template>
		</ModuleHeader>
		<v-container fluid>
				...
				<v-col cols="12">
					...
					>
											...
											<v-row no-gutters>
												<v-col xs="12" sm="6" md="6">
													<v-card flat>
														...
														<v-card-subtitle>
															{{
																$store.getters["uiadmin/getProdiName"](
																	formdata.prodi_id
																)
															}}
														</v-card-subtitle>
											...
											<v-row no-gutters>
												<v-col cols="12">
													<v-card>
														...
														<v-card-subtitle>
															Klik tombol berikut ini untuk mengirim ulang email
						...
						<template v-slot:item.nomor_hp="{ item }">
							{{
								item.nomor_hp == null || item.nomor_hp == ""
									? "N.A"
									: "+" + item.nomor_hp
							}}
						</template>
	...
	export default {
		...
		data: () => ({
			...
			headers: [
				...
				{ text: "NAMA MAHASISWA", value: "name", width: 300, sortable: true },
		...
		methods: {
			...
			aktifkan(id) {
				...
				this.$ajax
					.post(
						"/akademik/kemahasiswaan/updatestatus/" + id,
						{
							_method: "PUT",
							active: 1,
						},
