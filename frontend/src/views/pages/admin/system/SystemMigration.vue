				...
				<v-col cols="12">
					<v-form ref="frmdata" v-model="form_valid" lazy-validation>
						<v-card class="mb-4">
							...
							<v-card-text>
								<v-alert type="info">
									Isi form mahasiswa dengan lengkap dan benar; hasil inputan
									bisa dilihat di Akademik->Daftar Mahasiswa atau
							...
							<v-card-text>
								...
								>
									...
									<template v-slot:no-data>
										belum ada data tahun akademik dan semester, silahkan ganti
										Tahun Pendaftaran ke yang lebih kecil dari 2021
									</template>
								</v-data-table>
							</v-card-text>
						</v-card>
	...
	export default {
		...
		data: () => ({
			...
			rule_nirm: [
				value =>
					!!value ||
					"Nomor Induk Registrasi Masuk (NIRM) mohon untuk diisi !!!",
				value =>
					/^[0-9]+$/.test(value) ||
					"Nomor Induk Registrasi Masuk (NIRM) hanya boleh angka",
			],
			rule_nama_mhs: [
		...
		methods: {
			...
			initialize: async function() {
				...
				this.daftar_status_mhs = this.$store.getters[
					"uiadmin/getDaftarStatusMahasiswa"
				];
			},
			save() {
					...
					this.$ajax
						.post(
							"/system/migration/store",
							{
								...
								status_mhs: JSON.stringify(
									Object.assign({}, this.formdata.status_mhs)
								),
							},
							{
								headers: {
